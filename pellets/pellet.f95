...
!First load in variables from datin file.
       open(10,file='pellet.in', status='old')
...
!internal energy per unit mass (function of temperature)
        ...
        !we set mean conditions for the sacrificial region.
      gasinit(1,:,:) = 1.d0
      gasinit(2:4,:,:) = 0.0d0
      gasinit(5,:,:) =  1.0d0
...
!--------------------------------------------------------------------------------------------------------
!this code reads in sigma, this is the damping factor. A file named in variable fdamp is loaded into sigma, the damping factor. !!THE SIZE OF FDAMP MUST MATCH THE GRIDSIZE EXACTLY!!
       open(502,file='sigma200.in', status='old')
       do l=1,lt
       read(502,101)(sigma(l,m),m=1,mt)
       end do 
      open(503,file='sigma.out',status='unknown')
      do l = 1,lt
      write(503,101)(sigma(l,m),m=1,mt)
      end do
!--------------------------------------------------------------------------------------------------------
...
!density, sensible to set rho = 1, ie the density is equal to the normalized density everywhere
        vv(1,:,:,:) = 1.d0
       vv(1,0,125:275,125:275) = 1.0d0 + 1.d-1*spbomb(25:175,25:175)   
...
!       print*,vvt(1,200,3)
       ...
       do l = 1,lt
       do m = 1,mt
       vvt(1:5,l,m) = vvt(1:5,l,m)-sigma(l,m)*(vvt(1:5,l,m)-gasinit(1:5,l,m))  
       end do
       end do
!       print*,  vvt(1,200,3)

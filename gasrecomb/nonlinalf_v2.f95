...
!and the gas
      gasinit(1,:,:)    = 1.d0
      gasinit(2:4,:,:)  = 0.0d0
      gasinit(5,:,:)    = 0.d0+1.0d0
      print*,'e5=',(gamma*beta/(2d0*(gamma-1)))
...
elseif(experiment.eq.3) then
    print*, 'Driver is spbomb in both gas and plasma'
      open(101,file='spbomb.in', status='unknown')
        print*, 'debug11'
       do l=1,201
       read(101,101)(spbomb(l,m),m=1,199)
...
!with initial velocity field being 0 everywhere we assume that E0 = Eb+Ethermal then it is easy to express E as E0+Ek
       ...
       et = (uu(1,0,:,:)**(1.344))
       print*,'et=',et(252,301),'density=',uu(1,0,252,301)
       eb = 0.5d0*(uu(6,0,:,:)**2+uu(7,0,:,:)**2)
       print*,ek(252,301),et(252,301),eb(252,301),'energiees'
   
       etot(:,:) = (ek + et + eb)
       plasinit(5,:,:)=0+1**1.344+0.5d0
...
!since E0 = P0 for a cell with v = 0 then E = rho
       ...
       ek(:,:) = 0.5d0*(vv(1,0,:,:)*((vv(2,0,:,:)+vv(3,0,:,:))/vv(1,0,:,:)))**2     
       et = (vv(1,0,:,:)**(1.344))
       print*,'et=',et(252,301),'density=',vv(1,0,252,301)
...
!--------------------------------------------------------------
      ...
      Include 'nonlinalf.inc'
!This subroutine contains the finite difference solver. The richtmyer two-step scheme in 3 dimensions is used, this can be swapped out for any scheme that also only requires the  functions, call the flux functions.
        print*,'debugsolve' 
      do n = 1,nt
...
!kinetic energy density
      ek(:,:) = 0.5d0 * (vvt(2,:,:)**2+vvt(3,:,:)**2)/vvt(1,:,:)
!internal energy density
      eint(:,:) = vvt(5,:,:)-ek(:,:) 
!note that eint is really eint*rho but you never need the true value of e anyway
...
((nremaining/20*timediff)/60-floor((nremaining/20*timediff)/60))*60, &
 ' secs'
      ...
      do l=0,lt+1
       write(21,100)((uu(1,0,l,m)),',',m=0,lt+1)
      end do
      write(21,*)
      write(21,*)
      ...
      write(25,*)'# timestep = ',n
      do l=0,lt+1
      write(25,100)((uu(5,0,l,m)),',',m=0,lt+1)
      end do
      write(25,*)
      write(25,*)
      ...
      write(29,*)'# timestep = ',n
      do l=0,lt+1
      write(29,100)((vv(1,0,l,m)),',',m=0,lt+1)
      end do
      write(29,*)
      write(29,*)
      ...
      write(33,*)'# timestep = ',n
      do l=0,lt+1
      write(33,100)((vv(5,0,l,m)),',',m=0,lt+1)
      end do
      write(33,*)
      write(33,*)
